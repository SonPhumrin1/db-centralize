...
- [ ] **T-013** Add Better Auth to Next.js: configure `lib/auth.ts` with email+password, PostgreSQL adapter, `nextCookies()` plugin
- [ ] **T-014** Add Better Auth catch-all route: `app/api/auth/[...all]/route.ts`
- [ ] **T-015** Build login page (`app/(auth)/login/page.tsx`) — username + password form using Shadcn `Input` and `Button`
- [ ] **T-016** Add Next.js `middleware.ts` — protect all `/dashboard/*` routes, redirect to `/login` if no session
- [ ] **T-017** Write Go `SessionAuthMiddleware` — reads Better Auth session cookie, queries `sessions` table, sets `user_id` in Fiber locals. Returns 401 if invalid.
- [ ] **T-018** Add `GET /api/v1/me` (protected) — returns current user id, username, role. Use this to verify middleware works.
...
- [ ] **T-020** Create GORM model `DataSource` (id, user_id, name, type, config_encrypted, status, last_tested_at, created_at)
- [ ] **T-021** Write `crypto.Encrypt` / `crypto.Decrypt` using AES-GCM and `ENCRYPTION_KEY` from env
- [ ] **T-022** Define `DataSourceRepository` interface (FindAll, FindByID, Create, Delete) — all methods take `userID` param
- [ ] **T-023** Implement GORM `DataSourceRepository` — every query has `WHERE user_id = ?`
- [ ] **T-024** Implement `DataSourceUsecase` (list, create, delete, test)
- [ ] **T-025** Write connection test logic:
...
- [ ] **T-026** Write `GET /api/v1/datasources/:id/schema` — opens short-lived connection, queries `information_schema.columns`, returns `{ tables, columns }`. Cache result in Redis for 5 min.
- [ ] **T-027** Implement `DataSourceHandler` and register routes:
  - `GET    /api/v1/datasources`
  - `POST   /api/v1/datasources`
  - `DELETE /api/v1/datasources/:id`
  - `POST   /api/v1/datasources/:id/test`
  - `GET    /api/v1/datasources/:id/schema`
- [ ] **T-028** Build frontend sources page (`sources/page.tsx`): cards list, status dot, table name pills for DB sources, base URL for REST sources. "Add Source" button opens modal.
- [ ] **T-029** Build "Add Data Source" modal — form fields change based on type dropdown (PostgreSQL / MySQL / REST API). REST section shows auth type dropdown + dynamic fields. "Test Connection" button before save.
- [ ] **T-030** Wire frontend with TanStack Query — `useQuery` for list + schema, `useMutation` for create / delete / test
...
- [ ] **T-031** Create GORM model `Query` (id, user_id, data_source_id, name, body, created_at, updated_at)
- [ ] **T-032** Define and implement `QueryRepository` interface (FindAll, FindByID, Create, Update, Delete) — always scoped by `user_id`
- [ ] **T-033** Write `RunAgainstSource(ctx, source, queryBody)` for DB sources:
...
- [ ] **T-034** Write `RESTAdapter.Fetch(ctx, source, path)`:
...
- [ ] **T-035** Implement `QueryUsecase` (list, create, update, delete, run)
- [ ] **T-036** In `QueryUsecase.Create`: after saving query, auto-create an inactive `Endpoint` with a unique slug (see T-042 for slug logic)
- [ ] **T-037** Implement `QueryHandler` and register routes:
  - `GET    /api/v1/queries`
  - `POST   /api/v1/queries`
  - `PUT    /api/v1/queries/:id`
  - `DELETE /api/v1/queries/:id`
  - `POST   /api/v1/queries/:id/run`
- [ ] **T-038** Build frontend query page (`queries/page.tsx`):
...
- [ ] **T-040** Create GORM model `Endpoint` (id, user_id, query_id nullable, pipeline_id nullable, name, slug unique, is_active, created_at)
- [ ] **T-041** Define and implement `EndpointRepository` (FindAll, FindByID, FindBySlug, Create, Update, Delete, SlugExists) — FindAll scoped by `user_id`, FindBySlug NOT scoped (slug is global)
- [ ] **T-042** Write `generateUniqueSlug(name, repo)`:
  - slugify name ("My Query" → "my-query")
  - check if slug exists via `repo.SlugExists`
  - if exists, try "my-query-1", "my-query-2" until unique
- [ ] **T-043** Write `InvokeAuthMiddleware`:
...
- [ ] **T-044** Implement `EndpointHandler` and register routes:
...
- [ ] **T-045** Implement `/invoke/:slug` handler — runs the linked query, returns `[]map[string]any` as JSON
- [ ] **T-046** Build frontend endpoint page (`endpoints/page.tsx`):
...
- [ ] **T-050** Create GORM models: `Pipeline` (id, user_id, name, canvas_json, created_at, updated_at), `PipelineRun` (id, pipeline_id, status, result_snapshot, ran_at)
- [ ] **T-051** Define and implement `PipelineRepository` — scoped by `user_id`
- [ ] **T-052** Write `PipelineExecutor`:
...
- [ ] **T-053** Write unit tests for `PipelineExecutor` — test each node type with hardcoded mock `[]map[string]any` rows. No DB needed.
- [ ] **T-054** Implement `PipelineHandler` and register routes:
...
- [ ] **T-055** Install React Flow: `bun add @xyflow/react`
- [ ] **T-056** Build pipeline list page (`pipelines/page.tsx`): cards with name, last run status, last run time. "New Pipeline" button.
- [ ] **T-057** Build pipeline canvas page (`pipelines/[id]/canvas/page.tsx`):
...
- [ ] **T-058** Build config drawer components:
...
- [ ] **T-059** Wire Save button → `PUT /api/v1/pipelines/:id` with serialized canvas JSON
- [ ] **T-060** Wire Run button → `POST /api/v1/pipelines/:id/run` → update Output node config with result rows, show row count badge
- [ ] **T-061** Save each run to `pipeline_runs` table with status + result snapshot
...
- [ ] **T-070** Build dashboard home (`page.tsx`):
...
- [ ] **T-071** Click data source card → source detail page: full schema table (table name, columns, types)
- [ ] **T-072** Build sidebar navigation: Dashboard, Sources, Queries, Endpoints, Pipelines, Settings (admin only)
- [ ] **T-073** Build User Management page (admin only): list users, create user form, change role, deactivate toggle
- [ ] **T-074** Build System Settings page (admin only): platform name, pagination size, change root password
- [ ] **T-075** Add Shadcn `Skeleton` loading states to all list pages
- [ ] **T-076** Add Shadcn `Sonner` toast notifications — success and error toasts on all mutations
- [ ] **T-077** Add confirmation dialogs (Shadcn `AlertDialog`) for: delete source, delete query, delete endpoint, delete pipeline, deactivate user
- [ ] **T-078** Full end-to-end test: login → add source → write query → save → activate endpoint → call with curl → build pipeline → run pipeline
...
- [ ] **T-080** Add request body validation to all Go handlers — return `400` with field-level error messages for missing/invalid inputs
- [ ] **T-081** Add rate limiting to `/invoke/:slug` route (Fiber built-in limiter — e.g. 60 req/min per IP)
- [ ] **T-082** Add CORS config — allow only `NEXT_PUBLIC_APP_URL` origin
- [ ] **T-083** Write unit tests for all repository and usecase layers
- [ ] **T-084** Write isolation tests: user A creates resources, verify user B gets 403 on all of them
- [ ] **T-085** Add structured request logging (Fiber logger middleware)
- [ ] **T-086** Write `README.md`: local setup steps, env var descriptions, how to run tests, how to call an endpoint with curl
